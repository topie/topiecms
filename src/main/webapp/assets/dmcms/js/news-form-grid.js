...
function flushGrid()
{
	...
	if (currentChannelType == '0') {
		...
		grid = $("#content_grid").dmGrid(getnewsoption(action,hasPublishRole));
	}
...
/** **********普通新闻表格选项*************** */
function getnewsoption(action,hasPublishRole){
var actionColoums =[{
	text : "预览",
	cls : "green btn-sm",
	icon : "fa fa-search",
	...
	handle : function(index, data) {
		...
		modal.show();*/
		$("#content_grid").html("");
		form = $("#content_grid").dmForm(getForm(data.contentType,hasPublishRole));
...
// form
/** **************普通内容表单选项*************** */
function getForm(contentType,hasPublishRole) {
	var buttons = [];
	if(hasPublishRole){
		buttons.push(
		{
